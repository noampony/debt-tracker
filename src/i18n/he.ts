export const ui = {
  app: {
    title: "החזרתי?",
    subtitle: "מעקב חובות בין חברים ומשפחה",
  },
  actions: {
    addMember: "הוספת איש קשר",
    newTransaction: "תנועה חדשה",
    save: "שמירה",
    cancel: "ביטול",
    back: "חזרה",
    view: "צפייה",
    addTransaction: "תנועה חדשה",
    viewDetails: "פרטים",
    edit: "עריכה",
    delete: "מחיקה",
  },
  home: {
    todayLabel: "היום",
    quickActionTitle: "רישום חוב מהיר",
    quickActionDescription: "בחר איש קשר, הזן סכום וסיבה, ושמור את התנועה במקום.",
  },
  overview: {
    title: "סיכום חובות",
    totalOwedToUser: "סה״כ חייבים לך",
    totalUserOwes: "סה״כ אתה חייב",
  },
  members: {
    title: "אנשי קשר",
    loading: "טוען אנשי קשר",
    empty: "אין אנשי קשר עדיין",
    addTitle: "הוספת איש קשר",
    nameLabel: "שם",
    nameRequired: "יש להזין שם",
    duplicateName: "איש קשר בשם הזה כבר קיים",
    formHelp: "שם איש הקשר יוצג ברשימת החובות.",
    detailsPending: "מסך פרטי איש קשר יושלם בהמשך",
    detailTitle: "פרטי איש קשר",
    currentBalance: "יתרה נוכחית",
    editName: "שינוי שם",
    editTitle: "עריכת שם",
    deleteName: "מחיקת איש קשר",
    deleteConfirmTitle: "מחיקת איש קשר",
    deleteConfirmBody:
      "מחיקת {memberName} וכל היסטוריית התנועות שלו/ה. פעולה זו אינה ניתנת לביטול. האם להמשיך?",
    deleteConfirm: "מחיקה",
    resetDebt: "איפוס חוב",
    resetHelp: "האיפוס יוסיף תנועת איזון להיסטוריה.",
    resetDisabled: "אין חוב פתוח לאיפוס.",
    resetDialogTitle: "איפוס חוב",
    resetDialogBody: "הפעולה תאפס את החוב מול {memberName} ותוסיף תנועת איזון להיסטוריה. האם להמשיך?",
    resetDialogConfirm: "אישור איפוס",
    resetDialogCancel: "ביטול",
  },
  transaction: {
    addTitle: "תנועה חדשה",
    memberLabel: "איש קשר",
    memberPlaceholder: "בחירת איש קשר",
    amountLabel: "סכום",
    amountPlaceholder: "0",
    directionLabel: "סוג התנועה",
    memberOwesUserLabel: "הוא חייב לי",
    userOwesMemberLabel: "אני חייב לו",
    memberReturnedToUserLabel: "הוא החזיר לי",
    userReturnedToMemberLabel: "אני החזרתי לו",
    dateLabel: "תאריך",
    reasonLabel: "סיבה",
    reasonPlaceholder: "לדוגמה: סופר",
    commonReasons: ["אוכל", "נסיעה", "קניות", "מתנה", "שכר דירה", "החזר", "אחר"],
    notesLabel: "הערות",
    notesPlaceholder: "פרטים נוספים, אם צריך",
    memberRequired: "יש לבחור איש קשר",
    amountRequired: "יש להזין סכום",
    amountInvalid: "יש להזין סכום תקין",
    amountZero: "הסכום חייב להיות גדול מאפס",
    amountNegative: "הסכום לא יכול להיות שלילי",
    reasonRequired: "יש להזין סיבה",
    dateRequired: "יש לבחור תאריך",
    historyTitle: "היסטוריית תנועות",
    historyEmpty: "אין עדיין תנועות מול איש הקשר הזה",
    editTransactionTitle: "עריכת תנועה",
    deleteTransactionConfirmTitle: "מחיקת תנועה",
    deleteTransactionConfirmBody: "האם למחוק את התנועה? פעולה זו אינה ניתנת לביטול.",
    deleteTransactionConfirm: "מחיקה",
    dateLabelShort: "תאריך",
    amountLabelShort: "סכום",
    reasonLabelShort: "סיבה",
    notesLabelShort: "הערות",
  },
  auth: {
    loginTitle: "כניסה לחשבון",
    registerTitle: "יצירת חשבון",
    emailLabel: "כתובת מייל",
    emailPlaceholder: "your@email.com",
    passwordLabel: "סיסמה",
    passwordPlaceholder: "לפחות 8 תווים",
    loginButton: "כניסה",
    registerButton: "הרשמה",
    switchToRegister: "אין לך חשבון? הרשמה",
    switchToLogin: "כבר רשום/ה? כניסה",
    logoutButton: "יציאה",
    errorInvalidCredentials: "האימייל או הסיסמה שגויים",
    errorEmailTaken: "כתובת המייל כבר רשומה במערכת",
    errorGeneric: "אירעה שגיאה. נסה/י שנית.",
  },
  loading: {
    main: "טוען נתונים...",
    savingMember: "שומר...",
    savingTransaction: "שומר תנועה...",
    savingEditTransaction: "שומר תנועה...",
    resetting: "מאפס...",
    deletingMember: "מוחק...",
    deletingTransaction: "מוחק...",
  },
  error: {
    loadFailed: "שגיאה בטעינת הנתונים. נסה/י לרענן את הדף.",
    memberCreateFailed: "שגיאה בשמירת איש הקשר. נסה/י שנית.",
    memberUpdateFailed: "שגיאה בעדכון השם. נסה/י שנית.",
    memberDeleteFailed: "שגיאה במחיקת איש הקשר. נסה/י שנית.",
    transactionCreateFailed: "שגיאה בשמירת התנועה. נסה/י שנית.",
    transactionUpdateFailed: "שגיאה בעדכון התנועה. נסה/י שנית.",
    transactionDeleteFailed: "שגיאה במחיקת התנועה. נסה/י שנית.",
    resetFailed: "שגיאה באיפוס החוב. נסה/י שנית.",
  },
} as const;
